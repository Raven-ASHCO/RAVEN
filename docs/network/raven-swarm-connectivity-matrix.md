...
|------------------|---------|---------------------------|------------------------|
| `raven_core::internet` / InternetTransport (RIH1 hello + length-prefix) — ADR-0002 V1 first land | **lab localhost indexed delivery** (software); **fail-closed** for legacy `raven-node run`; **not** WAN | Codec: `node/crates/raven-core/src/internet.rs`. Live sockets: `node/crates/raven-node/src/internet_direct.rs`. Positive lab: `node/scripts/internet_indexed_two_node.sh` (CI `rust-linux` / `rust-macos` “InternetTransport localhost indexed two-node”). Negative: `node/scripts/internet_dial_smoke.sh` (legacy run still `ATSAM_SESSION_REQUIRED`). Gate: `INTERNET_DIRECT_PRODUCTION_ENABLED=false`; debug `RAVEN_LAB_TEST_A=1`. | **dial≠WAN.** No CI job dials a non-`127.0.0.1` / non-RFC1918 peer. Multi-NAT / DCUtR still **BLOCKED_HARDWARE**. Production slice flag stays false. Legacy `run` still refuses before connect without a persisted ATSAM session. |

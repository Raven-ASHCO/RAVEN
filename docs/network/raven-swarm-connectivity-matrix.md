...
|------------------|---------|---------------------------|------------------------|
| `raven_core::internet` / InternetTransport (RIH1 hello + length-prefix) — ADR-0002 V1 first land | **lab indexed delivery** on loopback **and** same-host non-loopback IPv4 (software); **fail-closed** for legacy `raven-node run`; **not** WAN | Codec: `node/crates/raven-core/src/internet.rs`. Live sockets: `node/crates/raven-node/src/internet_direct.rs`. Positive lab: `node/scripts/internet_indexed_two_node.sh` (`INTERNET_INDEXED_LOOPBACK_PASS` + `INTERNET_INDEXED_NON_LOOPBACK_PASS`). Negative: `node/scripts/internet_dial_smoke.sh`. Gate: `INTERNET_DIRECT_PRODUCTION_ENABLED=false`; debug `RAVEN_LAB_TEST_A=1`. | **dial≠WAN.** Same-host non-loopback / RFC1918 ≠ public-Internet peer. No CI job dials a distinct public WAN host. Multi-NAT / DCUtR still **BLOCKED_HARDWARE**. Named-pipe ≠ WAN. Production slice flag stays false. |

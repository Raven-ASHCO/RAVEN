...
| **G-M3** | Two-device RDAP harness **lab-only** | **Sole M3 vehicle:** [RAVEN#57](https://github.com/Raven-ASHCO/RAVEN/pull/57). Localhost two-process execute: [`o6-m3-two-node-rdap-lab-evidence.md`](o6-m3-two-node-rdap-lab-evidence.md). D5 full acceptance remains **RED**: no RDAP `ask` over `atsam_rvn1`, no carrier enum, physical WAN = `BLOCKED_HARDWARE`. SRE bar: wall-clock OK; one bring-up retry max; no silent skip; soft p50/p95 only after a real snapshot. **Not** O6 E2E Proven. **Not** HOLD lift. No duplicate harness PRs. | Raven↔RDAP (#17) + SRE Perf (#19); consult Adversarial QA (#18) + Eng Program + CLI DX |
...
| Open a second M3 harness PR | Manager SoT: [RAVEN#57](https://github.com/Raven-ASHCO/RAVEN/pull/57) is the sole M3 vehicle (#17 + #19). |

...
| InternetTransport indexed (lab) | **localhost + same-host non-loopback software Proven (lab)** — RIH1 + PairInit + sealed ACK via `internet_indexed_two_node.sh`. **dial≠WAN.** RFC1918/same-host ≠ public WAN. multi-NAT **BLOCKED_HARDWARE**. Production flag stays false. Named-pipe ≠ WAN. |
...
| **direct** (Path C) | **Locked** — LAN/localhost **proven**; InternetTransport indexed **lab loopback + same-host non-loopback proven**; WAN **blocked/untested** | **Path C lock:** legacy internet dial **fail-closed proven** — **NOT** a WAN reliability claim. Lab smoke: `internet_indexed_two_node.sh` = RIH1 + indexed ACK on `127.0.0.1` and a same-host NIC IPv4 (**dial≠WAN**; RFC1918 ≠ public WAN). multi-NAT **BLOCKED_HARDWARE**. `ash_menu_smoke` → CLI DX [RAVEN#16](https://github.com/Raven-ASHCO/RAVEN/pull/16). Windows **honest fail-closed** until named-pipe. Named-pipe/UDS work **does not unblock WAN**. | WAN Proven would need executed green/red on a **distinct non-loopback / non-RFC1918 public peer**. Same-host NIC IPv4 and `127.0.0.1` (B12) are **not** WAN Proven. |

...
#
# CLAIM (exact):
#   PASS = localhost/lab InternetTransport indexed two-node delivery (RIH1 hello
#   + framed PairInit/message + sealed ACK). dial≠WAN. Not public-Internet,
#   not multi-NAT, not WAN Proven.
...
fail() {
  ...
  echo "CLAIM: fail is localhost/lab InternetTransport — still not WAN Proven" >&2
  exit 1
}
...
echo "=== start two raven-node service processes (internet listen; LAN bind unused) ==="
...
echo "CLAIM: INTERNET_DIRECT_PRODUCTION_ENABLED=false; global PRODUCTION_ENABLED flags unchanged"
